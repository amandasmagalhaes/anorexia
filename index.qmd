...
Estimar a prevalência e características sociodemográficas associadas ao risco de anorexia.
...
A amostra inicial incluiu 10.773 indivíduos. Foram excluídos 153 participantes com resposta "não se aplica" ou "não sabe/não respondeu" para risco de anorexia e 2.176 indivíduos com menos de 60 anos de idade, resultando em uma amostra analítica final de 8.444 participantes.
...
# Prevalência anorexia

A prevalência do risco de anorexia na população estudada foi de 28,2% (IC95%: 24,4–32,4).
...
tbl_regression(
  ...
  modify_caption(
    "**Tabela 5. Associação entre características sociodemográficas e risco de anorexia**"
  ) |>
  modify_footnote(
    everything() ~ NA
  ) |>
  ...
  gt::tab_source_note(
    gt::md(
      "OR ajustada obtida por regressão logística considerando simultaneamente faixa etária, sexo, raça/cor, escolaridade, estado civil, renda domiciliar per capita, área de residência e macrorregião geográfica."
    )
  )
```