...
df <- df |>
  mutate(
    satisfeito = case_when(
      ...
      n73_4 == 2 ~ "Se sente cheio/satisfeito depois de comer cerca de um terço de uma refeição",
      n73_4 == 3 ~ "Se sente cheio/satisfeito depois de comer mais da metade da refeição",
      n73_4 == 4 ~ "Se sente cheio/satisfeito depois de comer a maior parte da refeição",
      n73_4 == 5 ~ "Quase nunca se sente cheio/satisfeito",
      n73_4 %in% c(8, 9) ~ NA_character_
    ),
    satisfeito = factor(
      satisfeito,
      levels = c("Se sente cheio/satisfeito depois de comer apenas alguns bocados", 
      "Se sente cheio/satisfeito depois de comer cerca de um terço de uma refeição", 
      "Se sente cheio/satisfeito depois de comer mais da metade da refeição", 
      "Se sente cheio/satisfeito depois de comer a maior parte da refeição", 
      "Quase nunca se sente cheio/satisfeito")
    )
  )
df |>
  count(satisfeito) |>
...
#
#
#| label: Descritiva refeição
#| code-summary: Descritiva refeição
df <- df |>
  mutate(
    refeicao = case_when(
      n73_5 == 1 ~ "Menos de uma refeição por dia",
      n73_5 == 2 ~ "Uma refeição por dia",
      n73_5 == 3 ~ "Duas refeições por dia",
      n73_5 == 4 ~ "Três refeições por dia",
      n73_5 == 5 ~ "Mais de três refeições por dia",
      n73_5 %in% c(8, 9) ~ NA_character_
    ),
    refeicao = factor(
      refeicao,
      levels = c("Menos de uma refeição por dia", 
      "Uma refeição por dia", 
      "Duas refeições por dia", 
      "Três refeições por dia", 
      "Mais de três refeições por dia")
    )
  )
df |>
  count(refeicao) |>
...
#| code-summary: Descritiva anorexia
#| results: hide
df <- df |>
  mutate(
    snaq = rowSums(
      data.frame(
        as.numeric(apetite),
        as.numeric(gosto_comida),
        as.numeric(satisfeito),
        as.numeric(refeicao)
      ),
      na.rm = FALSE
    ),
    snaq_cat = factor(
      if_else(snaq <= 14, "≤14", ">14", missing = NA_character_),
      levels = c(">14", "≤14")
    )
  )
df |>
...
#
#
#| label: Remover "não se aplica" ou "não sabe/não respondeu" (n = 153)
#| code-summary: Remover "não se aplica" ou "não sabe/não respondeu" (n = 153)
df <- df |>
  filter(!is.na(snaq_cat))
#
#
#
#
...
#
#
#| label: Remover menor que 60 anos (n = 2176)
#| code-summary: Remover menor que 60 anos (n = 2176)
df <- df |>
  filter(idade >= 60)
#
#
...
#
#
#| label: Prevalência anorexia
#| code-summary: Prevalência anorexia
prev_anorexia <- svyciprop(
  ~ I(snaq_cat == "≤14"),
  desenho,
  method = "logit"
)
prev <- coef(prev_anorexia)[1] * 100
ic <- confint(prev_anorexia) * 100
tibble(
  Indicador = "Risco de anorexia",
...
tbl_svysummary(
  ...
  modify_spanning_header(
    c(stat_1, stat_2) ~ "**Risco de anorexia**"
  ) |>
  modify_footnote(
  ...
  modify_caption(
    "**Tabela 2. Características da amostra total e segundo risco de anorexia**"
  )
#
#
#
...
tab_prev |>
  ...
  tab_header(
    title = md(
      "**Tabela 3. Prevalência do risco de anorexia segundo características sociodemográficas**"
    )
  ) |>
  cols_label(
...
resultado_uni |>
  ...
  tab_header(
    title = md(
      "**Tabela 4. Regressão logística univariada para risco de anorexia**"
    )
  ) |>
  cols_label(
    Caracteristica = md("**Características**"),
